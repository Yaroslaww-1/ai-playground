...
class DQN:
    def __init__(self, params):
        ...
        self.x = tf.placeholder('float', [None, params['width'], params['height'], 4], name=self.network_name + '_x')
        ...
        self.yj = tf.add(self.rewards, tf.multiply(1.0-self.terminals, tf.multiply(self.discount, self.q_t))) #rewards + gamma * max(Q)
        self.Q_pred = tf.reduce_sum(tf.multiply(self.y,self.actions), reduction_indices=1)
        self.cost = tf.reduce_sum(tf.pow(tf.subtract(self.yj, self.Q_pred), 2)) #Deep Deterministic Policy Gradient - https://stats.stackexchange.com/questions/534024/in-a-dqn-why-not-train-q-against-actual-discounted-rewards
        print(self.cost)
    ...
    def train(self, bat_s, bat_a, bat_t, bat_n, bat_r):
        ...
        q_predict = self.sess.run(self.y, feed_dict=feed_dict)
        q_predict = np.amax(q_predict, axis=1)
        feed_dict = {
            self.x: bat_s,
            self.q_t: q_predict,  # now we have predicted value, so we can use gamma + max(Q) equation
            self.actions: bat_a,
            self.terminals: bat_t,
            self.rewards: bat_r
        }
